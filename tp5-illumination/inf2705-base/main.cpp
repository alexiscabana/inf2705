...
static void animer( int tempsPrec )
{
   ...
   if ( enmouvement )
   {
      const float freqRotX  = 0.01;     // la fréquence de rotation en X
      const float freqRotY  = 0.005;    // la fréquence de rotation en Y
      const float freqRotZ  = 0.01;     // la fréquence de rotation en Z
      angleRotX += deltaTemps * 360.0 * ( freqRotX / FPS );
      angleRotY += deltaTemps * 360.0 * ( freqRotY / FPS );
      angleRotZ += deltaTemps * 360.0 * ( freqRotZ / FPS );
